...
class PTreeOpt():
  ...
  def __init__(self, f, scsplit, feature_bounds, discrete_actions = False, action_bounds = None, 
               action_names = None, population_size = 100, mu = 15, max_depth = 5,
    ...
    self.num_obj = len(epsilons)
    self.scsplit = scsplit
    self.worker_per = len(scsplit) -1 
  ...
  def run(self, max_nfe=100, parallel=False, log_frequency=None, filename='file'):
    ...
    if is_master:
      ...
      if log_frequency:
        snapshots = {'nfe': [], 'time': [], 'best_f': [], 'best_P': [], 'population': [], 'objectives': []}
    else:
    ...
    while nfe < max_nfe:
      ...
      # evaluate objectives
      if not parallel:
        self.objectives = np.array([self.f(P,self.scsplit[0],self.scsplit[1]) for P in self.population])
      else:
        if is_master:
          chunks = np.tile(self.population,self.worker_per)
          chunks = np.array_split(chunks, size)
        else:
          chunks = None
        try:
          local_Ps = comm.scatter(chunks, root=0)
        except:
          # for some reason this is empty sometimes. Print error and move on.
          print('ERROR: EMPTY CHUNK ON PROC. %d' % rank)
        for i in range(self.worker_per):
          if (comm.rank > self.popsize * i - 1) and (comm.rank <= (self.popsize * (i + 1) - 1)):
            local_fs = [self.f(P,self.scsplit[i],self.scsplit[i+1]) for P in local_Ps]
        comm.barrier()
        objs = comm.gather(local_fs, root=0)
        comm.barrier()
        if is_master:
          objs = np.sum(objs, axis = 1)
          obj_sum = np.empty([int(size/self.worker_per),self.num_obj])
          for i in range(int(size/self.worker_per)):
            obj_sum[i] = sum(objs[i::int(size/self.worker_per)])
          self.objectives = obj_sum
      nfe += self.popsize
      ...
      if is_master:
        ...
        if log_frequency is not None and nfe >= last_log + log_frequency:
          ...
          if not self.multiobj:
            ...
            # print(self.objectives) # only for now, to debug
          else:
            print('# nfe = %d\n%s' % (nfe, self.best_f)) 
          ...
          snapshots['population'].append(self.population)
          snapshots['objectives'].append(self.objectives)

          last_log = nfe
          pickle.dump(snapshots, open('snapshots/%s.pkl'%filename, 'wb'))
