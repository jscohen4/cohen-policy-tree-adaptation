...
**3.4.3 Action timing**: `action-hits.py` simulates each robust policy over each testing scenario and stores the years each action is triggered in the `action-hits` directory. `figure-scripts/Figure-9-action-timing.py` uses this data to generate **Figure 9**.
