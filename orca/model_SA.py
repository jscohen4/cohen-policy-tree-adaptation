...
class Model_SA(): 
	...
	def __init__(self, P, df, dfind, Y, min_depth, hist_datafile,SHA_baseline, ORO_baseline, FOL_baseline,SA_cost_params,baseline_run = False):
	...
	########################################################################################         
		...
		############################################################################   
		self.Levee_SA_mult = SA_cost_params[0]
		self.Dam_SA_mult = SA_cost_params[1]
		self.Offstream_SA_mult = SA_cost_params[2]
		self.Demand_SA_mult = SA_cost_params[3]
		self.GW_SA_mult = SA_cost_params[4]
	...
	###################################################################################################  
		...
		for t in range(1,self.T):
			...
			if dowy == 1:
				...
				self.conservation_cost[t]=self.Demand_SA_mult*(swp_cost + cvp_cost + nodd_cost)*(1/(1+self.rdiscount[t]))**(y)
				...
				if any([policy =='Levee_1', policy =='Levee_2', policy =='Levee_3', policy =='Levee_4', policy =='Levee_5']):
					if Levee_levels[policy] == False:
						...
						if all([y + self.levee_lag < self.Y,self.sha_levee_expand[y] > self.shasta.max_outflow]):
							...
							self.build_cost[t] =  self.Levee_SA_mult*(expand_cost)*(1/(1+self.rdiscount[t]))**(y) #update cost of expansion. Extra maintenance cost added in post-processing
	        ...
	        ############################################################################################
				...
				if policy == 'Dam_1':
					if Dam_levels[policy] == False:
						...
						if all([y + self.res_lag < self.Y,self.sha_res_expand[y] > self.shasta.capacity]):
							self.build_cost[t] =  self.Dam_SA_mult*(expand_cost)*(1/(1+self.rdiscount[t]))**(y) #update cost of expansion. Extra maintenance cost added in post-processing
				...
				if all([self.offstream_build_cap[y-self.offstream_lag] == self.offstream.capacity_1, self.offstream_1 == False, self.offstream_2 == False, self.offstream_3 == False]):
					...
					self.build_cost[t-365*self.offstream_lag] =  self.Offstream_SA_mult*(self.offstream.cost_1)*(1/(1+self.rdiscount[t]))**(y)
					...
					self.build_cost[t-365*self.offstream_lag] =  self.Offstream_SA_mult*(self.offstream.cost_2)*(1/(1+self.rdiscount[t]))**(y)
					...
					self.offstream.in_capacity = self.offstream.in_capacity_2
					self.offstream.out_capacity = self.offstream.out_capacity_2
					self.build_cost[t-365*self.offstream_lag] = self.Offstream_SA_mult*(self.offstream.cost_expand_1_2)*(1/(1+self.rdiscount[t]))**(y)
					...
					self.build_cost[t-365*self.offstream_lag] = self.Offstream_SA_mult*(self.offstream.cost_3)*(1/(1+self.rdiscount[t]))**(y)
					...
					self.offstream.in_capacity = self.offstream.in_capacity_3
					self.offstream.out_capacity = self.offstream.out_capacity_3
					self.build_cost[t] = self.Offstream_SA_mult*(self.offstream.cost_expand_1_3)*(1/(1+self.rdiscount[t]))**(y)
					...
					self.offstream.in_capacity = self.offstream.in_capacity_3
					self.offstream.out_capacity = self.offstream.out_capacity_3
					self.build_cost[t-365*self.offstream_lag] = self.Offstream_SA_mult*(self.offstream.cost_expand_2_3)*(1/(1+self.rdiscount[t]))**(y)
					offstream = True
					self.offstream_3 = True
	...
	#offstream option 3
				...
				#############################################################################################################################  
				self.maintenance_cost[t] += self.Levee_SA_mult*(self.folsom.max_outflow-150000)*self.folsom.levee_maint_cost
				self.maintenance_cost[t] += self.Levee_SA_mult*(self.oroville.max_outflow-79000)*self.oroville.levee_maint_cost
				self.maintenance_cost[t] += self.Levee_SA_mult*(self.shasta.max_outflow-115000)*self.shasta.levee_maint_cost
				self.maintenance_cost[t] += self.Dam_SA_mult*(self.folsom.capacity-975)*self.folsom.dam_maint_cost
				self.maintenance_cost[t] += self.Dam_SA_mult*(self.shasta.capacity-4552)*self.shasta.dam_maint_cost
				self.maintenance_cost[t] += self.Offstream_SA_mult*(self.offstream.capacity)*self.offstream.maint_cost
			...
			self.shasta.gw_cost = self.shasta.gw_cost*self.GW_SA_mult
			self.oroville.gw_cost = self.oroville.gw_cost*self.GW_SA_mult
			self.folsom.gw_cost = self.folsom.gw_cost*self.GW_SA_mult
